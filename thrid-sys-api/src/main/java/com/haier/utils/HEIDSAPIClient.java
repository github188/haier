...
public class HEIDSAPIClient {
	...
	private static final Logger logger = LoggerFactory.getLogger(HEIDSAPIClient.class);
	 ...
	 */
	public HEIDSAPIClient(String serviceUrl, String secretKey,
...
//		maps.put("data", encryptedData);
//		methodPost.addRequestHeader("Content-Type", "application/json; charset=utf-8");
//		methodPost.setRequestBody(encryptedData);
...
//			logger.debug("final to post ids request is [" + methodPost + "] !");
		...
		try {
			responsePost=HEHttpClients.httpPostStream(serviceUrl,encryptedData);
		} catch (Exception e) {
			e.printStackTrace();
